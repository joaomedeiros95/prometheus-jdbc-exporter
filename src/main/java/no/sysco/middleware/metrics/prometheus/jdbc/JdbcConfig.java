...
class JdbcConfig {
    ...
    JdbcConfig(Map<String, Object> yamlConfig) {
        ...
        if (yamlConfig.containsKey("drivers")) {
            final List<String> drivers = (List<String>) yamlConfig.get("drivers");
            for (String driver : drivers) {
                try {
                    DriverManager.registerDriver((Driver) Class.forName(driver).newInstance());
                } catch (ClassNotFoundException e) {
                    throw new IllegalArgumentException("Wrong driver informed", e);
                } catch (IllegalAccessException | InstantiationException | SQLException e) {
                    throw new IllegalArgumentException("Failed to start driver", e);
                }
            }
        }
                ...
                if (jobObject.containsKey("connections")) {
                    ...
                    for (Map<String, Object> connObject : connections) {
                        ...
                        if (containsUsername && !containsPassword) {
                            throw new IllegalArgumentException("If you specify a username you need to specify a " +
                                    "password");
                        }

                        if (!containsUsername && containsPassword) {
                            throw new IllegalArgumentException("If you specify a password you need to specify a " +
                                    "username");
                        }
                                    ...
                                    return
                                            job.queries
                                                    ...
                                                    .flatMap(query -> {
                                                        try {
                                                            ResultSet rs;
                                                            if (connection.prepareStatementSupport) {
                                                                PreparedStatement statement = conn.prepareStatement(
                                                                        query.query);
                                                                rs = statement.executeQuery();
                                                            } else {
                                                                rs = conn.createStatement().executeQuery(query.query);
                                                            }
    ...
    private static class JdbcConnection {
        ...
        String username;
        String password;
        boolean prepareStatementSupport;
    }
